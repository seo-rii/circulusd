...
- U10.5: the provisioned-host external run is done (dev host); the acceptance
  ...
  plus `go test -race -shuffle=on -count=5` green on the host-independent contract
  packages (`internal/agent`, the workerd harness, `cmd/agentd`); TypeScript
  `pnpm check` 5/5, `pnpm lint` clean, `pnpm test:unit` 507 passed. Every
  enumerated Unit 10 exit criterion is now satisfied on the dev host.
