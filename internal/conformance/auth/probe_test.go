...
func (probe fakeProbe) RunCheck(_ context.Context, check Check) (CheckOutcome, error) {
	...
	case probe.failID:
		return CheckOutcome{Passed: false, Detail: "credentials were regenerated in memory on restart"}, nil
	default:
		return CheckOutcome{Passed: true}, nil
	}
}

// doctorRequiredAuthComponents is the per-check component set internal/doctor's
// production ConformanceProfile requires for durable credentials. This gate must
// produce exactly this set so the doctor profile cannot require a component nothing
// emits.
var doctorRequiredAuthComponents = []string{
...
func TestQualifyReportWithoutProbeIsUnavailablePerComponent(t *testing.T) {
	...
	for _, component := range doctorRequiredAuthComponents {
	...
	if len(report.Results) != len(doctorRequiredAuthComponents) {
		t.Fatalf("report has %d results, want %d", len(report.Results), len(doctorRequiredAuthComponents))
	}
...
func TestQualifyReportReferenceProbePassesButIsNotPromotable(t *testing.T) {
	...
	profile := conformance.Profile{Name: "production", Production: true, Required: doctorRequiredAuthComponents}
	if err := collector.Evaluate(profile); err == nil {
		t.Fatal("production profile accepted a reference/mock credentials report")
	}
}
...
func TestQualifyReportNonReferenceProbeSatisfiesProductionProfile(t *testing.T) {
	...
	profile := conformance.Profile{Name: "production", Production: true, Required: doctorRequiredAuthComponents}
	if err := collector.Evaluate(profile); err != nil {
		t.Fatalf("production profile rejected an external credentials report: %v", err)
	}
}

func TestQualifyReportFailIsFail(t *testing.T) {
	t.Parallel()
	report := QualifyReport(context.Background(), fakeProbe{failID: "durable-credentials"})
	failed := index(report)["auth.durable-credentials"]
	if failed.Status != conformance.Fail {
		t.Fatalf("auth.durable-credentials = %s, want FAIL", failed.Status)
	}
	if !strings.Contains(failed.Reason, "durable-credentials") {
...
func TestQualifyReportProbeErrorIsUnavailable(t *testing.T) {
	t.Parallel()
	report := QualifyReport(context.Background(), fakeProbe{errorID: "durable-credentials"})
	errored := index(report)["auth.durable-credentials"]
	if errored.Status != conformance.Unavailable {
		t.Fatalf("auth.durable-credentials = %s, want UNAVAILABLE", errored.Status)
	}
	if !strings.Contains(errored.Reason, "durable-credentials") {
...
func TestRequiredChecksMatchDoctorComponentSet(t *testing.T) {
	...
	want := append([]string(nil), doctorRequiredAuthComponents...)
	sort.Strings(want)
