...
func (probe fakeProbe) RunCheck(_ context.Context, check Check) (CheckOutcome, error) {
	...
	case probe.failID:
		return CheckOutcome{Passed: false, Detail: "found a secret temp file surviving after use"}, nil
	default:
...
func TestQualifyReferenceProbePassesButIsNotPromotable(t *testing.T) {
	...
	if err := collector.Evaluate(profile); err == nil {
		t.Fatal("production profile accepted a reference/mock secret PASS")
	}
}
...
func TestQualifyFailsOnFailedCheck(t *testing.T) {
	t.Parallel()
	result := Qualify(context.Background(), fakeProbe{failID: "temp-file-cleanup"})
	if result.Status != conformance.Fail {
		t.Fatalf("result = %+v, want FAIL", result)
	}
	if !strings.Contains(result.Reason, "temp-file-cleanup") {
...
func TestQualifyUnavailableWhenCheckErrors(t *testing.T) {
	t.Parallel()
	result := Qualify(context.Background(), fakeProbe{errorID: "exposure-class-reuse-policy"})
	if result.Status != conformance.Unavailable {
		t.Fatalf("result = %+v, want UNAVAILABLE", result)
	}
	if !strings.Contains(result.Reason, "exposure-class-reuse-policy") {
		t.Fatalf("reason %q should name the check that could not run", result.Reason)
	}
	mustCollect(t, result)
}

func TestRequiredChecksCoverSecretContract(t *testing.T) {
	t.Parallel()
	want := map[string]bool{
		"proxy-only-raw-absent":       false,
		"sandbox-secret-use-audited":  false,
		"temp-file-cleanup":           false,
		"exposure-class-reuse-policy": false,
	}
	...
	for id, covered := range want {
		if !covered {
			t.Fatalf("required secret check %q is missing", id)
		}
	}
