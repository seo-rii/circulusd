...
func (probe fakeProbe) RunCheck(_ context.Context, check Check) (CheckOutcome, error) {
	...
	case probe.failID:
		return CheckOutcome{Passed: false, Detail: "checkpoint signature did not verify"}, nil
	default:
		return CheckOutcome{Passed: true}, nil
	}
}

// doctorRequiredAuditComponents is the per-check component set internal/doctor's
// production ConformanceProfile requires for the durable audit sink. This gate
// must produce exactly this set so the doctor profile cannot require a component
// nothing emits.
var doctorRequiredAuditComponents = []string{
	"audit.durable-chain",
...
func TestQualifyReportWithoutProbeIsUnavailablePerComponent(t *testing.T) {
	...
	for _, component := range doctorRequiredAuditComponents {
	...
	if len(report.Results) != len(doctorRequiredAuditComponents) {
		t.Fatalf("report has %d results, want %d", len(report.Results), len(doctorRequiredAuditComponents))
	}
...
func TestQualifyReportReferenceProbePassesButIsNotPromotable(t *testing.T) {
	...
	profile := conformance.Profile{Name: "production", Production: true, Required: doctorRequiredAuditComponents}
	if err := collector.Evaluate(profile); err == nil {
		t.Fatal("production profile accepted a reference/mock audit report")
	}
}
...
func TestQualifyReportNonReferenceProbeSatisfiesProductionProfile(t *testing.T) {
	...
	profile := conformance.Profile{Name: "production", Production: true, Required: doctorRequiredAuditComponents}
	if err := collector.Evaluate(profile); err != nil {
		t.Fatalf("production profile rejected an external audit report: %v", err)
	}
}

func TestQualifyReportFailsOnlyTheFailedComponent(t *testing.T) {
	t.Parallel()
	report := QualifyReport(context.Background(), fakeProbe{failID: "signed-checkpoint"})
	byComponent := index(report)
	failed := byComponent["audit.signed-checkpoint"]
	if failed.Status != conformance.Fail {
		t.Fatalf("audit.signed-checkpoint = %s, want FAIL", failed.Status)
	}
	if !strings.Contains(failed.Reason, "signed-checkpoint") {
		t.Fatalf("reason %q should name the failed check", failed.Reason)
	}
	chain := byComponent["audit.durable-chain"]
	if chain.Status != conformance.Pass {
		t.Fatalf("audit.durable-chain = %s, want PASS (a single failed property must not mask the others)", chain.Status)
	}
	mustCollect(t, report)
}
...
func TestQualifyReportUnavailableOnlyForTheErroredComponent(t *testing.T) {
	t.Parallel()
	report := QualifyReport(context.Background(), fakeProbe{errorID: "durable-chain"})
	byComponent := index(report)
	errored := byComponent["audit.durable-chain"]
	if errored.Status != conformance.Unavailable {
		t.Fatalf("audit.durable-chain = %s, want UNAVAILABLE", errored.Status)
	}
	if !strings.Contains(errored.Reason, "durable-chain") {
		t.Fatalf("reason %q should name the check that could not run", errored.Reason)
	}
	checkpoint := byComponent["audit.signed-checkpoint"]
	if checkpoint.Status != conformance.Pass {
		t.Fatalf("audit.signed-checkpoint = %s, want PASS", checkpoint.Status)
	}
	mustCollect(t, report)
}
...
func TestRequiredChecksMatchDoctorComponentSet(t *testing.T) {
	...
	want := append([]string(nil), doctorRequiredAuditComponents...)
	sort.Strings(want)
