...
func (probe fakeProbe) RunCheck(_ context.Context, check Check) (CheckOutcome, error) {
	...
	case probe.failID:
		return CheckOutcome{Passed: false, Detail: "a symlink escaping the workspace was committed"}, nil
	default:
...
func TestQualifyReferenceProbePassesButIsNotPromotable(t *testing.T) {
	...
	if err := collector.Evaluate(profile); err == nil {
		t.Fatal("production profile accepted a reference/mock filesystem PASS")
	}
}
...
func TestQualifyFailsOnFailedCheck(t *testing.T) {
	t.Parallel()
	result := Qualify(context.Background(), fakeProbe{failID: "escaping-symlink-rejected"})
	if result.Status != conformance.Fail {
		t.Fatalf("result = %+v, want FAIL", result)
	}
	if !strings.Contains(result.Reason, "escaping-symlink-rejected") {
...
func TestQualifyUnavailableWhenCheckErrors(t *testing.T) {
	t.Parallel()
	result := Qualify(context.Background(), fakeProbe{errorID: "overlay-fullscan-diff-equal"})
	if result.Status != conformance.Unavailable {
		t.Fatalf("result = %+v, want UNAVAILABLE", result)
	}
	if !strings.Contains(result.Reason, "overlay-fullscan-diff-equal") {
		t.Fatalf("reason %q should name the check that could not run", result.Reason)
	}
	mustCollect(t, result)
}

func TestRequiredChecksCoverFilesystemContract(t *testing.T) {
	t.Parallel()
	want := map[string]bool{
		"direct-write-durable-revision":  false,
		"manifest-diff-reflects-fs-ops":  false,
		"absolute-symlink-rejected":      false,
		"escaping-symlink-rejected":      false,
		"special-file-commit-rejected":   false,
		"large-blob-upload":              false,
		"sandbox-kill-restores-revision": false,
		"stale-sandbox-commit-rejected":  false,
		"single-mutable-writer-lease":    false,
		"no-lease-write-blocked":         false,
		"overlay-fullscan-diff-equal":    false,
	}
	...
	for id, covered := range want {
		if !covered {
			t.Fatalf("required filesystem check %q is missing", id)
		}
	}
