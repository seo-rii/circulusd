...
func (probe fakeProbe) RunCheck(_ context.Context, check Check) (CheckOutcome, error) {
	...
	case probe.failID:
		return CheckOutcome{Passed: false, Detail: "a cross-tenant resource ID granted access"}, nil
	default:
...
func TestQualifyReferenceProbePassesButIsNotPromotable(t *testing.T) {
	...
	if err := collector.Evaluate(profile); err == nil {
		t.Fatal("production profile accepted a reference/mock acl/quota PASS")
	}
}
...
func TestQualifyFailsOnFailedCheck(t *testing.T) {
	t.Parallel()
	result := Qualify(context.Background(), fakeProbe{failID: "cross-tenant-access-denied"})
	if result.Status != conformance.Fail {
		t.Fatalf("result = %+v, want FAIL", result)
	}
	if !strings.Contains(result.Reason, "cross-tenant-access-denied") {
...
func TestQualifyUnavailableWhenCheckErrors(t *testing.T) {
	t.Parallel()
	result := Qualify(context.Background(), fakeProbe{errorID: "quota-rejection-no-partial-mutation"})
	if result.Status != conformance.Unavailable {
		t.Fatalf("result = %+v, want UNAVAILABLE", result)
	}
	if !strings.Contains(result.Reason, "quota-rejection-no-partial-mutation") {
		t.Fatalf("reason %q should name the check that could not run", result.Reason)
	}
	mustCollect(t, result)
}

func TestRequiredChecksCoverAclQuotaContract(t *testing.T) {
	t.Parallel()
	want := map[string]bool{
		"cross-tenant-access-denied":          false,
		"workspace-role-enforced":             false,
		"quota-exceeded-admission-denied":     false,
		"quota-rejection-no-partial-mutation": false,
	}
	...
	for id, covered := range want {
		if !covered {
			t.Fatalf("required acl/quota check %q is missing", id)
		}
	}
