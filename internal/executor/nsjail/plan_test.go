...
func TestPlannerBuildsExplicitFailClosedProductionPlan(t *testing.T) {
	...
	plan, err := planner.Build(validRequest())
	if err != nil {
	...
	wantArguments := []string{
		"--config",
		"/run/circulusd/sandboxes/sbx_01/nsjail.pbtxt",
	}
	...
	required := []string{
		...
		"cgroupv2_mount: \"/sys/fs/cgroup/circulusd\"",
		...
		"src: \"/run/circulusd/sandboxes/sbx_01/workspace\"\n  dst: \"/workspace\"\n  is_bind: true\n  rw: false",
		"src: \"/run/circulusd/sandboxes/sbx_01/control\"\n  dst: \"/run/circulusd/control\"\n  is_bind: true\n  rw: true",
...
func TestPlannerSeparatesReadOnlyAndWritableWorkspacePlans(t *testing.T) {
	...
	readOnly, err := planner.Build(validRequest())
	if err != nil {
		t.Fatalf("Build(read-only) error = %v", err)
	}
	writableRequest := validRequest()
	...
	workspacePrefix := "src: \"/run/circulusd/sandboxes/sbx_01/workspace\"\n  dst: \"/workspace\"\n  is_bind: true\n  rw: "
	...
	readOnly.Arguments[0] = "mutated"
	readOnly.Configuration[0] = 'X'
	again, err := planner.Build(validRequest())
	if err != nil {
		t.Fatalf("Build(after caller mutation) error = %v", err)
	}
	if again.Arguments[0] != "--config" || again.Configuration[0] == 'X' {
		t.Fatal("caller mutation changed planner-owned state")
	}
}
...
func TestPlannerRejectsUntrustedPathsDowngradesAndUnboundedResources(t *testing.T) {
	...
	requestTests := []struct {
		...
		{name: "path sandbox ID", mutate: func(request *Request) { request.SandboxID = "../other" }},
...
func TestPlannerBuildIsDeterministicAndConcurrentSafe(t *testing.T) {
	...
	want, err := planner.Build(validRequest())
	if err != nil {
		t.Fatalf("Build() error = %v", err)
	}
	...
	for range workers {
		...
		go func() {
			defer wait.Done()
			plan, buildErr := planner.Build(validRequest())
			results <- plan
